...
while True:
    ...
    for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
        ...
        if matches[best_match_index]:
            ...
            # Fetch data from Firebase if there's a name match
            employee_data = db.child("attendance_records").order_by_child("employee_name").equal_to(name).get()
            
            for employee in employee_data.each():
                employee_dict = employee.val()
                if employee_dict.get("time_in") and not employee_dict.get("time_out"):
                    employee_dict["time_out"] = formatted_time
                    db.child("employee_name").child(employee.key()).update(employee_dict)
